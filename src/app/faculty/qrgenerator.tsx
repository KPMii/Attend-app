...
function CountdownRing({ seconds, total }: { seconds: number; total: number }) {
  ...
  return (
    <View style={{ width: size, height: size, alignItems: "center", justifyContent: "center" }}>
      ...
      <Text style={{ color: "rgba(255,255,255,0.4)", fontSize: 10, marginTop: 2 }}>sec</Text>
    </View>
  );
}
...
export default function QRGenerator() {
  ...
  const rotateToken = async () => {
    Animated.timing(qrFadeAnim, { toValue: 0, duration: 200, useNativeDriver: true }).start(async () => {
      ...
      Animated.timing(qrFadeAnim, { toValue: 1, duration: 300, useNativeDriver: true }).start();
  ...
  const syncColor =
    syncStatus === "synced" ? "#C8F04D" :
    syncStatus === "syncing" ? "#F2C14E" :
    syncStatus === "offline" ? "#F2816B" : "rgba(255,255,255,0.2)";

  const syncLabel =
    syncStatus === "synced" ? "● Synced to Database" :
    syncStatus === "syncing" ? "○ Syncing..." :
    syncStatus === "offline" ? "● Offline — queued" : "○ Not started";
        ...
        {!isActive ? (
          <View style={styles.form}>
            <View style={styles.subjectHeaderRow}>
              ...
              <TouchableOpacity onPress={() => router.push("/faculty/subjects")}>
            ...
            {subjects.length === 0 ? (
              ...
              >
                <Text style={styles.noSubjectsText}>No subjects yet — tap to add one</Text>
              </TouchableOpacity>
            ) : (
                  ...
                  >
                    <Text
                      style={[
                        ...
                        selectedSubjectId === s.id && styles.subjectChipTextActive,
                      ]}
                    >
                      {s.name}
                    </Text>
            ...
            {rooms.length === 0 ? (
              ...
              >
                <Text style={styles.noSubjectsText}>No rooms yet — tap to add one</Text>
              </TouchableOpacity>
            ) : (
              <View style={styles.subjectChipRow}>
                {rooms.map((r) => (
            ...
            <View style={styles.lateCard}>
              <View style={styles.lateCardHeader}>
                <Text style={styles.lateCardTitle}>Mark students late after</Text>
              ...
              <View style={styles.chipRow}>
                {[5, 10, 15, 20].map((mins) => (
                  <TouchableOpacity
                    key={mins}
                    style={[styles.chip, lateThreshold === mins && styles.chipActive]}
              ...
              <Text style={styles.lateCardHint}>
                Students scanning after this window will be marked late instead of present.
              </Text>
            </View>
          ...
          <View style={styles.activeSession}>
            <View style={styles.sessionInfo}>
              ...
              <View style={styles.sessionMeta}>
                ...
                <Text style={styles.sessionTimer}>{formatTime(sessionRemaining)}</Text>
              </View>
            </View>
            ...
            <Animated.View style={[styles.qrCard, { opacity: qrFadeAnim }]}>
              <View style={styles.qrInner}>
                {qrPayload ? (
                  <QRCode value={qrPayload} size={width * 0.58} color="#0D0D0D" backgroundColor="#FFFFFF" />
                ) : null}
              </View>
              <Text style={styles.qrHint}>Students scan this QR to mark attendance</Text>
            </Animated.View>
            ...
            <View style={styles.countdownRow}>
              ...
              <View style={styles.countdownInfo}>
                ...
                <Text style={styles.rotationCount}>Rotated {rotationCount}×</Text>
...
const styles = StyleSheet.create({
  ...
  headerTitle: { color: "#fff", fontSize: 30, fontWeight: "800", letterSpacing: -1 },
  ...
  subjectChipText: { color: "rgba(255,255,255,0.6)", fontSize: 13, fontWeight: "700" },
  ...
  lateCardHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  lateCardTitle: { color: "rgba(255,255,255,0.7)", fontSize: 13, fontWeight: "600" },
  ...
  lateCardHint: { color: "rgba(255,255,255,0.3)", fontSize: 11, lineHeight: 15 },
  ...
  sessionMeta: { flexDirection: "row", alignItems: "center", gap: 8, marginTop: 8 },
  ...
  tokenText: { color: "rgba(255,255,255,0.25)", fontSize: 11, fontFamily: "monospace" },
  stopBtn: {
    width: "100%",
    borderRadius: 16,
    borderWidth: 1,
